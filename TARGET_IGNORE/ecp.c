...
/* Enable Nuvoton's crypto ECC performance test */
#define NU_CRYPTO_ECC_PERFTST

#include "hal/us_ticker_api.h"

#if defined(NU_CRYPTO_ECC_PERFTST)

static uint32_t nu_perf_t1, nu_perf_t2, nu_perf_tdiff;

#define NU_PERFTST_BEGIN(FUNC)                        \
    do {                                                \
        nu_perf_t1 = us_ticker_read();                  \
    } while(0)

#define NU_PERFTST_END(FUNC)                          \
    do {                                                \
        nu_perf_t2 = us_ticker_read();                  \
        if (nu_perf_t2 >= nu_perf_t1) {                 \
            nu_perf_tdiff = nu_perf_t2 - nu_perf_t1;    \
        } else {                                        \
            nu_perf_tdiff = (uint64_t) nu_perf_t2       \
                            + 0x100000000               \
                            - nu_perf_t1;               \
        }                                               \
        mbedtls_printf("%s takes %d (us)\n",            \
                       #FUNC, nu_perf_tdiff);           \
    } while (0)
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    {
        mbedtls_ecp_point pt_;
        mbedtls_ecp_point_init( &pt_ );
        mbedtls_ecp_copy( &pt_, pt );
        NU_PERFTST_BEGIN(mbedtls_internal_ecp_normalize_jac);
        mbedtls_internal_ecp_normalize_jac( grp, &pt_ );
        NU_PERFTST_END(mbedtls_internal_ecp_normalize_jac);
        mbedtls_ecp_point_free( &pt_ );
    }
#else
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    NU_PERFTST_BEGIN(ecp_normalize_jac);
#endif
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    NU_PERFTST_END(ecp_normalize_jac);
#endif
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    {
        mbedtls_ecp_point **T_, *T__;
        T_ = mbedtls_calloc( T_size, sizeof( mbedtls_ecp_point *) );
        T__ = mbedtls_calloc( T_size, sizeof( mbedtls_ecp_point ) );
        for( i = 0; i < T_size; i++ ) {
            T_[i] = &T__[i];
            mbedtls_ecp_point_init( &T__[i] );
        }
        NU_PERFTST_BEGIN(mbedtls_internal_ecp_normalize_jac_many);
        mbedtls_internal_ecp_normalize_jac_many( grp, T_, T_size );
        NU_PERFTST_END(mbedtls_internal_ecp_normalize_jac_many);
        for( i = 0; i < T_size; i++ ) {
            mbedtls_ecp_point_free( &T__[i] );
        }
        mbedtls_free(T__);
        mbedtls_free(T_);
    }
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    NU_PERFTST_BEGIN(ecp_normalize_jac_many);
#endif
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    NU_PERFTST_END(ecp_normalize_jac_many);
#endif
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    {
        mbedtls_ecp_point R_;
        mbedtls_ecp_point_init( &R_ );
        NU_PERFTST_BEGIN(mbedtls_internal_ecp_double_jac);
        mbedtls_internal_ecp_double_jac( grp, &R_, P );
        NU_PERFTST_END(mbedtls_internal_ecp_double_jac);
        mbedtls_ecp_point_free( &R_ );
    }
#else
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    NU_PERFTST_BEGIN(ecp_double_jac);
#endif
                          ...
                          const mbedtls_ecp_point *P, const mbedtls_ecp_point *Q )
{
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    {
        mbedtls_ecp_point R_;
        mbedtls_ecp_point_init( &R_ );
        NU_PERFTST_BEGIN(mbedtls_internal_ecp_add_mixed);
        mbedtls_internal_ecp_add_mixed( grp, &R_, P, Q );
        NU_PERFTST_END(mbedtls_internal_ecp_add_mixed);
        mbedtls_ecp_point_free( &R_ );
    }
#else
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    NU_PERFTST_BEGIN(ecp_add_mixed);
#endif
                ...
                int (*f_rng)(void *, unsigned char *, size_t), void *p_rng )
{
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    {
        mbedtls_ecp_point pt_;
        mbedtls_ecp_point_init( &pt_ );
        NU_PERFTST_BEGIN(mbedtls_internal_ecp_randomize_jac);
        mbedtls_internal_ecp_randomize_jac( grp, &pt_, f_rng, p_rng );
        NU_PERFTST_END(mbedtls_internal_ecp_randomize_jac);
        mbedtls_ecp_point_free( &pt_ );
    }
#else
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    NU_PERFTST_BEGIN(ecp_randomize_jac);
#endif
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    NU_PERFTST_END(ecp_randomize_jac);
#endif
...
static int ecp_normalize_mxz( const mbedtls_ecp_group *grp, mbedtls_ecp_point *P )
{
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    {
        ...
        mbedtls_ecp_copy( &P_, P );
        NU_PERFTST_BEGIN(mbedtls_internal_ecp_normalize_mxz);
        mbedtls_internal_ecp_normalize_mxz( grp, &P_ );
        NU_PERFTST_END(mbedtls_internal_ecp_normalize_mxz);
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    NU_PERFTST_BEGIN(ecp_normalize_mxz);
#endif
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    NU_PERFTST_END(ecp_normalize_mxz);
#endif
                ...
                int (*f_rng)(void *, unsigned char *, size_t), void *p_rng )
{
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    {
        ...
        mbedtls_ecp_point_init( &P_ );
        mbedtls_ecp_copy( &P_, P );
        NU_PERFTST_BEGIN(mbedtls_internal_ecp_randomize_mxz);
        mbedtls_internal_ecp_randomize_mxz( grp, &P_, f_rng, p_rng );
        NU_PERFTST_END(mbedtls_internal_ecp_randomize_mxz);
        mbedtls_ecp_point_free( &P_ );
    }
#else
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    NU_PERFTST_BEGIN(ecp_randomize_mxz);
#endif
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    NU_PERFTST_END(ecp_randomize_mxz);
#endif
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    {
        mbedtls_ecp_point R_, S_;
        mbedtls_ecp_point_init( &R_ );
        mbedtls_ecp_point_init( &S_ );
        NU_PERFTST_BEGIN(mbedtls_internal_ecp_double_add_mxz);
        mbedtls_internal_ecp_double_add_mxz( grp, &R_, &S_, P, Q, d );
        NU_PERFTST_END(mbedtls_internal_ecp_double_add_mxz);
        mbedtls_ecp_point_free( &R_ );
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    NU_PERFTST_BEGIN(ecp_double_add_mxz);
#endif
...
#if defined(NU_CRYPTO_ECC_PERFTST)
    NU_PERFTST_END(ecp_double_add_mxz);
#endif
